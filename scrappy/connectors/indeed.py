...
class IndeedApiConnector:
    ...
    def discover_with_stats(
        ...
        if not self.endpoint:
            raise RuntimeError("Indeed API connector requires SCRAPPY_INDEED_API_URL")
    ...
    def offer_from_item(self, item: dict) -> Offer:
        title = _pick(item, "title", "jobTitle", "job_title", "name")
        ...
        remote = _pick(item, "remote", "workplaceType", "workplace_type")
        ...
        source_id = _pick(item, "id", "jobId", "job_id", "key", "jobkey", "indeedApplyJobId")
        if not source_id:
